...
def get_otp_capacity() -> str:
    """Retrieves the current capacity configuration of the OTP service, including max capacity, current user count, and latency settings."""
...
def scale_up_otp_capacity(new_capacity: int = 10) -> str:
    ...
    if not CONFIG_PATH.exists():
        config = {"otp_max_capacity": new_capacity, "current_user_count": 0, "simulate_latency_seconds": 3.0}
    else:
        with open(CONFIG_PATH, "r") as f:
            config = json.load(f)
